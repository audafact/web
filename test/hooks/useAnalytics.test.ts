...
describe("useAnalytics", () => {
  ...
  it("should provide feature response time tracking", () => {
    const { result } = renderHook(() => useAnalytics());
    const mockTrackFeatureResponseTime = vi.mocked(
      result.current.trackFeatureResponseTime
    );

    act(() => {
      const endTracking =
        result.current.trackFeatureResponseTime("test_feature");
  ...
  it("should handle funnel progress tracking", () => {
    const { result } = renderHook(() => useAnalytics());
    const mockTrackFunnelProgress = vi.mocked(
      result.current.trackFunnelProgress
    );
