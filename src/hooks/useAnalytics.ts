...
export const useAnalytics = () => {
  ...
  const trackEventWithTier = useCallback(
    <K extends keyof TrackingEvents>(
      event: K,
  ...
  const trackFeatureErrorWithContext = useCallback(
  ...
  const trackDemoSession = useCallback(
    (action: "started" | "ended", data?: any) => {
      if (action === "started") {
  ...
  const trackLibraryInteraction = useCallback(
    (action: "opened" | "previewed" | "added" | "searched", data: any) => {
      switch (action) {
        case "opened":
