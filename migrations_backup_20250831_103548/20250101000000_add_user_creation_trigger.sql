...
-- Note: This requires appropriate permissions on the auth schema

-- Check if we can create the trigger, if not, provide instructions
DO $$
BEGIN
  ...
  EXCEPTION WHEN insufficient_privilege THEN
    ...
    RAISE NOTICE 'This is normal in some environments. The function is created and ready.';
    RAISE NOTICE 'You may need to create the trigger manually or contact your database administrator.';
  EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'Error creating trigger: %', SQLERRM;
    RAISE NOTICE 'The handle_new_user function is created and ready.';
    RAISE NOTICE 'You may need to create the trigger manually.';
  END;
END $$;
